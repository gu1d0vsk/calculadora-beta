...
if not has_active_content:
    ...
    div.block-container {
        ...
        transition: transform 0.8s cubic-bezier(0.25, 1, 0.5, 1);
        padding-bottom: 120px; /* Garante que o conteúdo não fique atrás do rodapé */
    }
...
<style>
    ...
    /* --- RODAPÉ FIXO INVISÍVEL (CONTÉM TEXTO) --- */
    .fixed-bottom {{
        position: fixed;
        left: 0;
        bottom: 0;
        ...
        padding-bottom: 20px;
        z-index: 999;
        pointer-events: none; /* Permite clicar através do espaço vazio */
    }}
...
# Adiciona o elemento de teste que você pediu
html_content += "<p><em>Elemento novo no rodapé (Teste)</em></p>"

if html_content:
    st.markdown(f'<div class="fixed-bottom">{html_content}</div>', unsafe_allow_html=True)
